...
// ── Spreadsheet ID ───────────────────────────────────────────
// Production:
// var DESTINATION_FILEID = "1GEmrEzF0hZzpxgkdM6oqSyAAsEsilEJd-ew7Z8ryFoY";
// Dev:
//var DESTINATION_FILEID = "1bo5c68nTzgmj19WzTXZlnKi0JNQdBF3VRfK0uXOV4MQ";
...
function assignJob_(body) {
  ...
  sheet.getRange(r, COL.ESTIMATED_DUE_DATE).setValue(body.estimatedDueDate || "");
  return { success: true };
}
*/
function assignJob_(body) {
...
function attachFiles_(body) {
  ...
  files.forEach(function(file, idx) {
    try {
      ...
      console.warn("Permission error for " + file.fileId + ": " + e.message);
      // ไม่ block — บันทึก link ต่อแม้ตั้ง permission ไม่ได้
    }
